...
import 'package:pisti_app/services/api_service.dart'; // ApiService import yolu

class ProfileScreen extends StatefulWidget {
  final String userId; // 🎯 Dışarıdan giriş yapmış kullanıcının ID'sini alıyoruz
...
Future<void> _fetchFullProfileData() async {
  ...
  try {
    ...
    print("DEBUG - Gelen Profil Verisi: $profileData"); // 🎯 Hâlâ yazmıyorsa debug konsoluna bakın
...
Widget build(BuildContext context) {
  ...
  if (!_isLoading && _username.isNotEmpty && _username != "...") {
    ...
    if (parts.length > 1 && parts[1].isNotEmpty) {
      ...
      avatarText = _username.substring(0, _username.length >= 2 ? 2 : 1).toUpperCase();
    }
  }
              ...
              builder: (ctx, _) {
                ...
                if (_isLoading) {
                  return const SliverToBoxAdapter(
                    child: Padding(
                      ...
                      child: Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(kPrimary))),
                    ),
                  );
                }
                ...
                if (_tabController.index == 0) {
                  ...
                  return _myCreatedEvents.isEmpty
                      ? _buildEmptyStateSliver("Henüz bir etkinlik oluşturmadın.")
                      : _buildEventList(_myCreatedEvents);
                } else {
                  // KATILDIĞIM SEKME LİSTESİ
                  return _myJoinedEvents.isEmpty
                      ? _buildEmptyStateSliver("Henüz hiçbir etkinliğe katılmadın.")
  ...
  return Column(
    children: [
      ...
      Center(
        child: Stack(
          ...
          children: [
            ...
            CircleAvatar(
              radius: 50,
              backgroundColor: _isLoading ? kCard : kPrimary, // Yüklenirken soft gri bir halka olur
              child: CircleAvatar(
                radius: 47,
                backgroundColor: kBg,
                child: _isLoading 
                    ? const SizedBox(
                        width: 22, height: 22,
                        child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(kTextSub)),
                      )
                    : Text(avatarText, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: kText)),
              ),
            ),
          ],
        ),
      ...
      _isLoading
          ...
          : Text(_username, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: kText)),
      ...
      _isLoading
          ...
          : Text(_email, style: const TextStyle(color: kTextSub, fontWeight: FontWeight.w600, fontSize: 13)),
    ],
  );
}
    ...
    return Column(
      ...
      children: [
        const Padding(
          ...
          child: Text(
            "BAŞARILAR",
            style: TextStyle(color: kTextSub, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1),
          ),
        ),
        SizedBox(
          height: 110,
  ...
  Widget _buildEventList(List<Map<String, dynamic>> data) {
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        ...
        (ctx, i) {
          final String id = (data[i]['_id'] ?? data[i]['id'] ?? i.toString()).toString();
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(event['title'] ?? 'Başlıksız Etkinlik', style: const TextStyle(fontWeight: FontWeight.w900, color: kText, fontSize: 15)),
                  const SizedBox(height: 4),
                  Text(event['time'] ?? 'Zaman Belirtilmedi', style: const TextStyle(color: kTextSub, fontSize: 12, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
      ...
      child: Padding(
        ...
        child: Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.calendar_today_rounded, size: 40, color: kTextSub.withValues(alpha: 0.2)),
              const SizedBox(height: 12),
              Text(message, style: const TextStyle(color: kTextSub, fontSize: 13, fontWeight: FontWeight.w600)),
            ],
          ),
        ),
      ),
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(event['title'] ?? 'Etkinlik', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: kText)),
                      Text(event['time'] ?? '', style: const TextStyle(color: kPrimary, fontWeight: FontWeight.w800)),
                    ],
                  ),
                ),
              ],
            ...
            const SizedBox(height: 30),
            _infoRow(Icons.location_on_rounded, event['location'] ?? event['loc'] ?? 'Belirtilmedi'),
            const SizedBox(height: 12),
            _infoRow(Icons.people_alt_rounded, "${event['joined'] ?? 0} Katılımcı"),
            const SizedBox(height: 30),
            const Text("Etkinlik Hakkında", style: TextStyle(color: kText, fontWeight: FontWeight.w900, fontSize: 16)),
            const SizedBox(height: 10),
            Text(event['description'] ?? event['desc'] ?? 'Açıklama bulunmuyor.', style: TextStyle(color: kText.withValues(alpha: 0.6), height: 1.5)),
            const Spacer(),
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text("KAPAT", style: TextStyle(fontWeight: FontWeight.w900, color: Colors.white, fontSize: 16)),
              ),
            ),
          ],
        ),
